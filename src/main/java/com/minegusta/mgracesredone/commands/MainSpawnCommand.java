...
public class MainSpawnCommand implements CommandExecutor {
	@Override
	public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
		...
		if (args.length > 1 && sender.hasPermission("minegusta.spawn.other")) {
			...
			try {
				...
				if (WorldCheck.isEnabled(p.getWorld())) p.teleport(SpawnLocationUtil.getSpawn(RaceType.HUMAN));
		...
		if (sender instanceof Player && sender.hasPermission("minegusta.spawn")) {
			if (!WorldCheck.isEnabled(((Player) sender).getWorld())) {
				...
				((Player) sender).teleport(SpawnLocationUtil.getSpawn(RaceType.HUMAN));
			}
